...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Guardar reference en la orden para poder hacer match desde el webhook
    await (prisma as any).productOrder.update({
      where: { id: productOrder.id },
      data: { boldReference: reference },
    });
    ...
    if (!boldResponse.ok) {
      ...
      // Si falla Bold, eliminar la orden creada
    ...
    // Guardar info de todos los productos en sessionStorage para el webhook
