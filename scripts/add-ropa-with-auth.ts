// Script para agregar productos de ropa mediante la API local con autenticación
const ropaProducts = [
  {
...
ropaProducts.forEach((product, index) => {
  ...
  console.log(`   Precio: $${product.precio.toLocaleString('es-CO')}`);
  ...
  console.log(`   Descripción: ${product.descripcion.substring(0, 80)}...`);
  console.log('');
});

console.log('=== Instrucciones para Agregar ===');
console.log('1. Inicia sesión como administrador en la aplicación');
console.log('2. Ve al panel de administración de productos');
console.log('3. Agrega manualmente estos productos o usa la API con autenticación');
console.log('');
console.log('=== Datos Listos para Importar ===');
console.log(JSON.stringify(ropaProducts, null, 2));
